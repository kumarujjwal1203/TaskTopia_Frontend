      ...
      .filter((task) =>
        ["true", "1", "yes"].includes(
          typeof task.completed === "string"
            ...
            : String(task.completed).toLowerCase()
        )
      )
      .sort((a, b) => {
        switch (sortBy) {
      ...
      <div className={CT_CLASSES.header}>
        <div className={CT_CLASSES.titleWrapper}>
          <h1 className={CT_CLASSES.title}>
            <CheckCircle2 className="text-violet-500 w-5 h-5 md:w-6 md:h-6" />
        ...
        {sortedCompletedTasks.length === 0 ? (
          <div className={CT_CLASSES.emptyState}>
            <div className={CT_CLASSES.emptyIconWrapper}>
              <CheckCircle2 className="w-6 h-6 md:w-8 md:h-8 tect-violet-500" />
            </div>
